...
export default function Hero() {
  ...
  return <section id="top" className="relative isolate flex min-h-screen items-center overflow-hidden">
    ...
    <Container className="relative z-20 pt-24 max-md:pb-24 max-md:pt-48">
      <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[.2em] text-slate-300"><span className="bg-pfa-accent h-px w-10" />Premier Football Agency</motion.div>
      ...
      <motion.div initial={false} animate={{ opacity: 1, y: 0 }} className="mt-10 flex gap-4 max-sm:flex-col"><Button href="/players">Наши игроки</Button><Button href="/contacts" variant="secondary">Связаться</Button></motion.div>
    </Container>
    <a href="#agency" className="absolute bottom-9 left-12 z-20 flex items-center gap-4 text-[9px] uppercase tracking-[.18em] text-slate-300 max-md:hidden">Подробнее <ArrowDown className="text-pfa-accent" size={18} /></a>
