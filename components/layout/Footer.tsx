...
export default function Footer() {
  return <footer className="border-t border-white/10 py-16"><Container className="grid grid-cols-2 gap-12 max-md:grid-cols-1">
    ...
    <nav className="grid grid-cols-2 gap-4 justify-self-end text-xs max-md:justify-self-start" aria-label="Навигация в подвале">{NAVIGATION.map((item) => <Link className="transition hover:text-[#22C55E]" key={item.href} href={item.href}>{item.label}</Link>)}</nav>
