...
export default function Header() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 grid h-24 grid-cols-[1fr_auto_1fr] items-center px-12 transition-all duration-500 max-lg:grid-cols-[1fr_auto] max-lg:px-5 ${isScrolled ? "h-[76px] border-b border-white/10 bg-[#050B14]/85 backdrop-blur-xl" : ""}`}>
    ...
    <AnimatePresence>{isOpen && <motion.div id="mobile-navigation" role="dialog" aria-modal="true" aria-label="Мобильная навигация" className="fixed inset-0 z-50 flex flex-col bg-[#050B14] p-8" initial={{ clipPath: "inset(0 0 100% 0)" }} animate={{ clipPath: "inset(0)" }} exit={{ clipPath: "inset(0 0 100% 0)" }} transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}>
      ...
      <nav className="flex flex-1 flex-col justify-center gap-3" aria-label="Мобильная навигация">{NAVIGATION.map((item) => <Link key={item.href} href={item.href} aria-current={isCurrentPage(item.href) ? "page" : undefined} onClick={closeMenu} className="font-display text-[clamp(3.5rem,15vw,6rem)] uppercase leading-none aria-[current=page]:text-[#22C55E]">{item.label}</Link>)}</nav>
